...
# NOTE: axim._nanochat.common computes COMPUTE_DTYPE at import time from the
# NANOCHAT_DTYPE env var / CUDA capability, so we must set the env var BEFORE
# importing anything from axim._nanochat. We parse args first (pure stdlib) to
# get --dtype, set the env, then import axim._nanochat.
...
def main():
    ...
    # ---- set dtype env BEFORE importing axim._nanochat --------------------
    if C["dtype"]:
        os.environ["NANOCHAT_DTYPE"] = C["dtype"]

    # ---- now import the heavy stuff ---------------------------------------
    from axim._nanochat.common import (compute_init, compute_cleanup, print0,
                                 autodetect_device_type, COMPUTE_DTYPE,
                                 COMPUTE_DTYPE_REASON, is_ddp_initialized)
    from axim._nanochat.gpt import GPT, GPTConfig
    from axim.sft.data import (AximTokenizer, load_jsonl, prepare_examples,
...
def _resume(model, resume_from: str, device, master: bool) -> int:
    ...
    from axim._nanochat.common import print0
    p = Path(resume_from)
    if p.is_dir():
...
def _eval_loss(model, val_ex, C, tokenizer, device, world_size):
    ...
    from axim.sft.data import PackedBatcher
    model.eval()
...
def _sample(model, tokenizer, val_rows, C, device, max_seq_len, step, static_rows):
    ...
    from axim.sft.data import normalize_record
    from axim._nanochat.common import print0
    model.eval()
    import random
...
def _save_axim(path, model, cfg, enc, base_meta, step, val_loss, C, master=False):
    ...
    from axim._nanochat.common import is_ddp_initialized
